...
namespace ReversiAspDotNetCore
{
	...
	{
		#region デリゲート
		public delegate FuncsJson ViewMsgDlg(string title , string msg);
		public delegate FuncsJson DrawSingle(int y, int x, int sts, int bk, string text);
		public delegate FuncsJson CurColMsg(string text);
		public delegate FuncsJson CurStsMsg(string text);
		public delegate FuncsJson Wait(int time);
		#endregion
		...
		public int gameEndAnimExec()
		{
			...
			if (this.mSetting.mEndAnim == ReversiConst.DEF_END_ANIM_ON) {
				...
				this.WaitLocal(this.mSetting.mEndInterVal);
				// *** マス消去 *** //
				for (var i = 0; i < this.mSetting.mMasuCnt; i++) {
					for (var j = 0; j < this.mSetting.mMasuCnt; j++) {
				...
				for (var i = 0; i < this.mSetting.mMasuCnt; i++) {
					for (var j = 0; j < this.mSetting.mMasuCnt; j++) {
						...
						if (bEnd == 1 && wEnd == 1) {
							...
							this.WaitLocal(this.mSetting.mEndDrawInterVal);
						}
					}
				}
				ret = 0;
		...
		{
			if(this.viewMsgDlg != null) this.mCallbacks.funcs.Add(this.viewMsgDlg(title, msg));
		}
		...
		{
			if(this.drawSingle != null) this.mCallbacks.funcs.Add(this.drawSingle(y, x, sts, bk, text));
		}
		...
		{
			if(this.curColMsg != null) this.mCallbacks.funcs.Add(this.curColMsg(text));
		}
		...
		{
			if(this.curStsMsg != null) this.mCallbacks.funcs.Add(this.curStsMsg(text));
		}
		...
		{
			if(this.wait != null) this.mCallbacks.funcs.Add(this.wait(time));
		}
	}
}
