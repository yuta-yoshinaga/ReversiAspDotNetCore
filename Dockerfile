...
RUN dotnet publish "ReversiAspDotNetCore.csproj" -c Release -o /app/publish

FROM base AS final
WORKDIR /app
COPY --from=publish /app/publish .
#-- For heroku-container modification -- START
#ENTRYPOINT ["dotnet", "ReversiAspDotNetCore.dll"]
CMD ASPNETCORE_URLS=http://*:$PORT dotnet ReversiAspDotNetCore.dll
#-- For heroku-container modification -- END